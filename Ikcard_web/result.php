...
<body class="hold-transition skin-blue sidebar-mini">
    ...
    <br><br>
    <div class="content">
      <form class="form-inline" action="result.php?value=kkhj6877hhYTT77ggghjhgj778GGT" method="post">
      ...
      <?php if(isset($_GET['value']) AND $_GET['value']=='kkhj6877hhYTT77ggghjhgj778GGT'){ ?>
      <p class="alert alert-danger" style="text-align:center">Resultat sur "<?php echo htmlspecialchars($_POST['search']); ?>"</p>
    <?php } ?>
      ...
      <div class="table-responsive">
                  <table id="example2" class="table table-bordered table-hover" data-name="cool-table">
                    ...
                    <tbody>
                      <?php $req=$bdd->query('select * from particulier WHERE name_all LIKE "%'.$_POST['search'].'%"
                      || name_user LIKE "%'.$_POST['search'].'%"
                      || email LIKE "%'.$_POST['search'].'%"
                      || numero LIKE "%'.$_POST['search'].'%"
                      || poste_occupe LIKE "%'.$_POST['search'].'%"
                      || profession LIKE "%'.$_POST['search'].'%"
                      || adresse LIKE "%'.$_POST['search'].'%"
                      || compagnie LIKE "%'.$_POST['search'].'%"
                      || site_web LIKE "%'.$_POST['search'].'%" '); $count=0;
                      while ($rep=$req->fetch()) {
                        ?>
                      <tr>
                        ...
                        <td>
                          <a href="profile-particulier/profil_particulier.php?id=<?php echo $rep['id']; ?>"><i class="fa fa-eye"></i></a>
